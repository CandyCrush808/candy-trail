...
export function AmbientEffects({ theme }: { theme?: string }) {
  ...
  return (
    <div className={`ambient ambient-${theme ?? "garden"}`} aria-hidden="true">
      {Array.from({ length: 10 }, (_, i) => (
        <motion.i
          key={i}
          className="ambient-dot"
          style={{ left: `${8 + i * 9}%`, top: `${6 + (i * 17) % 85}%` }}
          {...animationProps}
...
export function XpToast({ amount, onDone }: { amount: number; onDone: () => void }) {
  ...
  return (
    <motion.div className="xp-toast" initial={{ opacity: 0, y: 15, scale: 0.8 }} animate={{ opacity: 1, y: -35, scale: 1 }} exit={{ opacity: 0 }}>
      {`+${amount} XP ✨`}
    </motion.div>
  );
}

export function Celebration({ title }: { title: string | null }) {
  return (
    <AnimatePresence>
      {title && (
